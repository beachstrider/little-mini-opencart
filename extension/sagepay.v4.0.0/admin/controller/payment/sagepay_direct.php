<?php
namespace Opencart\Admin\Controller\Extension\Sagepay\Payment;
class SagepayDirect extends \Opencart\System\Engine\Controller {
	...
	public function index(): void {
		...
		$data['breadcrumbs'][] = [
			...
			'href' => $this->url->link('extension/sagepay/payment/sagepay_direct', 'user_token=' . $this->session->data['user_token'])
		];
		
		$data['save'] = $this->url->link('extension/sagepay/payment/sagepay_direct|save', 'user_token=' . $this->session->data['user_token']);
		...
		$this->response->setOutput($this->load->view('extension/sagepay/payment/sagepay_direct', $data));
	}
	...
	public function install(): void {
		...
		$this->model_extension_sagepay_payment_sagepay_direct->install();
	}
	
	public function uninstall(): void {
		$this->load->model('extension/sagepay/payment/sagepay_direct');
		
		$this->model_extension_sagepay_payment_sagepay_direct->uninstall();
	}
	...
	public function order(): string {
		if ($this->config->get('payment_sagepay_direct_status')) {
			...
			$sagepay_direct_order = $this->model_extension_sagepay_payment_sagepay_direct->getOrder($this->request->get['order_id']);

			if (!empty($sagepay_direct_order)) {
				$this->load->language('extension/sagepay/payment/sagepay_direct');

				$sagepay_direct_order['total_released'] = $this->model_extension_sagepay_payment_sagepay_direct->getTotalReleased($sagepay_direct_order['sagepay_direct_order_id']);
				...
				return $this->load->view('extension/sagepay/payment/sagepay_direct_order', $data);
			}
		}
	}
	...
	public function void() {
		...
		if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '') {
			...
			$sagepay_direct_order = $this->model_extension_sagepay_payment_sagepay_direct->getOrder($this->request->post['order_id']);

			$void_response = $this->model_extension_sagepay_payment_sagepay_direct->void($this->request->post['order_id']);

			$this->model_extension_sagepay_payment_sagepay_direct->logger('Void result', $void_response);

			if (!empty($release_response) && $void_response['Status'] == 'OK') {
				$this->model_extension_sagepay_payment_sagepay_direct->addTransaction($sagepay_direct_order['sagepay_direct_order_id'], 'void', 0.00);
				$this->model_extension_sagepay_payment_sagepay_direct->updateVoidStatus($sagepay_direct_order['sagepay_direct_order_id'], 1);
	...
	public function release(): void {
		...
		if (isset($this->request->post['order_id']) && $this->request->post['order_id'] != '' && isset($this->request->post['amount']) && $this->request->post['amount'] > 0) {
			...
			$release_response = $this->model_extension_sagepay_payment_sagepay_direct->release($this->request->post['order_id'], $this->request->post['amount']);

			$this->model_extension_sagepay_payment_sagepay_direct->logger('Release result', $release_response);

			if (!empty($release_response) && $release_response['Status'] == 'OK') {
				$this->model_extension_sagepay_payment_sagepay_direct->addTransaction($sagepay_direct_order['sagepay_direct_order_id'], 'payment', $this->request->post['amount']);

				$total_released = $this->model_extension_sagepay_payment_sagepay_direct->getTotalReleased($sagepay_direct_order['sagepay_direct_order_id']);

				if ($total_released >= $sagepay_direct_order['total'] || $sagepay_direct_order['settle_type'] == 0) {
					$this->model_extension_sagepay_payment_sagepay_direct->updateReleaseStatus($sagepay_direct_order['sagepay_direct_order_id'], 1);
					$release_status = 1;
	...
	public function rebate(): void {
		...
		if (isset($this->request->post['order_id']) && !empty($this->request->post['order_id'])) {
			...
			$rebate_response = $this->model_extension_sagepay_payment_sagepay_direct->rebate($this->request->post['order_id'], $this->request->post['amount']);

			$this->model_extension_sagepay_payment_sagepay_direct->logger('Rebate result', $rebate_response);

			if (!empty($release_response) && $rebate_response['Status'] == 'OK') {
				$this->model_extension_sagepay_payment_sagepay_direct->addTransaction($sagepay_direct_order['sagepay_direct_order_id'], 'rebate', $this->request->post['amount'] * -1);

				$total_rebated = $this->model_extension_sagepay_payment_sagepay_direct->getTotalRebated($sagepay_direct_order['sagepay_direct_order_id']);
				$total_released = $this->model_extension_sagepay_payment_sagepay_direct->getTotalReleased($sagepay_direct_order['sagepay_direct_order_id']);

				if ($total_released <= 0 && $sagepay_direct_order['release_status'] == 1) {
					$this->model_extension_sagepay_payment_sagepay_direct->updateRebateStatus($sagepay_direct_order['sagepay_direct_order_id'], 1);
					$rebate_status = 1;
	...
	private function validate(): bool {
		if (!$this->user->hasPermission('modify', 'extension/sagepay/payment/sagepay_direct')) {
